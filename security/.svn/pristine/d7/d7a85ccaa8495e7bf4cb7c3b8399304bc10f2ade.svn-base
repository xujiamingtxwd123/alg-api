...
func NewPublicKeyRing(cap uint) *PublicKeyRing {
	return &PublicKeyRing{make([]*eda.PublicKey, 0, cap)}
}

// Add adds a public key, pub to the ring.
// All keys added to the ring must use the same curve.
func (r *PublicKeyRing) Add(pub ckey.Key) error{
	key,ok:=pub.(cecdsa.ECDSAPublicKey)
	if(!ok){
		return errors.New("invalid key type")
	}
	r.Ring = append(r.Ring, (key.GetECDSAPublicKey()))
	return nil
}
...
//func (k eda.PublicKey) String() string {
...
func (r *PublicKeyRing) Sign(privkey ckey.Key, m []byte) (rs *RingSign, err error) {

	pkey,ok:=privkey.(cecdsa.ECDSAPrivateKey)
	if(!ok){
		return nil,errors.New("invalid key type")
	}
	priv := pkey.GetECDSAPrivateKey()
	...
	for {
		...
		ct,_:=cecdsa.GetCurveType(pkey.GetCurveType())
		x,y:=ct.ScalarBaseMult(priv.D.Bytes())
		//
		//fmt.Println("Sign:D",priv.D)
		//fmt.Println("Sign:X",x)
		pub := eda.PublicKey{ct,x,y}
		curve := pub.Curve
		...
		for j := 0; j < s; j++ {
			...
			go func(j int) {
				defer wg.Done()
				c[j], err = randFieldElement(curve, rand.Reader)
				if err != nil {
					fmt.Println("error:Sign()-c[j]")
					return
				}
				t[j], err = randFieldElement(curve, rand.Reader)
				if err != nil {
					fmt.Println("error:Sign()-t[j]")
					return
				}
				//如果是签名者的公钥

				if ((bytes.Compare(r.Ring[j].X.Bytes(),pub.X.Bytes()) == 0) && (bytes.Compare(r.Ring[j].Y.Bytes(),pub.Y.Bytes()) == 0)) {
					id = j
					rb := t[j].Bytes()
		...
		result:=RingSign{hsx, hsy, c, t}
		if(r.Verify(m,&result)){
			return &result,nil
		}
	//	fmt.Println("wrong")
...
func (r *PublicKeyRing) Verify(m []byte, rs *RingSign) bool {
	s := r.Len()
	if s == 0 {
		return false
	}
